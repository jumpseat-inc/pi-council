...
### Step 10 — judge PASS at the merge candidate (job-13.8, re-dispatch)
Cycle-2 Skeptic at `4b470ec`: **NO-BLOCK, 7/7 closed-green** — mutation probe (b) (one-byte SEARCH_ROW_EMPTY via `sed` on driver.py) reddens frame 3 exit 1; `SMOKE_PHASE=6` Docker exit 0 + phase-6 PASS; host harness exit 0 + `SMOKE PASS` with root-owned dirs in place naming both in `prune:` warnings; tolerant-prune attacked via chmod-000 (old pipeline exit 123, fixed harness exit 0 + "pruned 0, left 2 in place"), frame assertions proven NOT softened (é falsifier green in a run where pruning could delete nothing); no-`SMOKE_PHASE` full path byte-identical (post-guard diff = 0); full gate set green at head. The first cycle-2 session was stall-killed by the 4-min no-activity monitor mid-Docker-run; re-dispatched with an extended stall window (quiet gate runs finish). Judge re-dispatched (the cycle-1 PASS was on record for the pre-fix head): **PASS** at `4b470ec` — all goal conjuncts verified, the judge itself ran `bash smoke/search-smoke/run.sh` exit 0 + 9 frames green and read the artifact frames (03 search-open, 06/07 filter, 08 Esc-clear restored, 09 no-match triple) plus the four conjuncts (documented procedure, CSI-u drive with stdlib-only driver, ruled copy byte-exact, re-execution record). 

...
## API: call OpenRouter, not TypeSafe's own API

**In this project, always call Jev through OpenRouter's Decisions endpoint.** Do not
call `api.typesafe.ai` and do not use the TypeSafe SDKs (they target TypeSafe's own
base URL and API key). The request body keeps the same `state` + `questions` shape the
TypeSafe docs describe; only the endpoint, auth, and model id differ.
...
```json
{
  ...
  "questions": {
    ...
    "team": {
      ...
      "criteria": { "account": "Login/permissions", "frontend": "Rendering", "payments": "Checkout/billing" }
    },
    "urgency": {
      "type": "score",
...
- **Model**: `typesafe/jev-1.13` (OpenRouter routes it to the `TypeSafe` provider).
- **Auth**: the standard OpenRouter Bearer key (`OPENROUTER_API_KEY`). Never hardcode it; keep it server-side.
- **Response**: `{ model, provider, answers, usage, id }`. Each `answers[id]` is a typed object: `noul` (`noul`), `choice` (`choice`, `probabilities`, `confidence`), or `score` (`score`, `legend`, `probabilities`, `confidence`).
- **This is not a chat-completions model.** Posting `typesafe/jev-1.13` to `/api/v1/chat/completions` will not work; use the Decisions route above.
- Endpoint reference: <https://openrouter.ai/docs/api/api-reference/alphadecisions/submit-a-decisions-questions-and-answers-request.md>
...
primitives, models, limits, and worked examples. They describe TypeSafe's *own* API
and SDKs — here you translate that guidance onto the OpenRouter Decisions endpoint in
the section above.
...
- Before writing an integration, read the current
  [OpenRouter Decisions endpoint reference](https://openrouter.ai/docs/api/api-reference/alphadecisions/submit-a-decisions-questions-and-answers-request.md)
  and the question guidance relevant to the design. For a new workflow, also inspect
  the closest cookbook: it often shows a better decomposition than a generic classifier.
...
| Write API code | [OpenRouter Decisions endpoint](https://openrouter.ai/docs/api/api-reference/alphadecisions/submit-a-decisions-questions-and-answers-request.md); body/response shape in "API: call OpenRouter" above |
| Update an older integration | [OpenRouter docs index](https://openrouter.ai/docs/llms.txt); TypeSafe's [migration guide](https://docs.typesafe.ai/migrating-to-v1.md) only if the old code called `api.typesafe.ai` directly |

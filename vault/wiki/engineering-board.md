...
The fold-in test was applied throughout — EV-78 (packaged data), EV-80 (state
packing), EV-81 (transport), EV-83 (routing), EV-84 (falsifier) each stayed their
own card while residuals were carded (`FLLWUP-96`…`104`). Step 13's follow-up
...
the judge reads. Cross-epic merges set `epic: null`. The second phase
**grouped** the 41 survivors into 7 thematic **close-out epics** (EPIC-16…22),
re-homing the `epic: null` orphans; a close-out epic's `goal` is an aggregate
rollup of its children rather than a single feature's sentence — a boundary with
the goal rule above. Witnesses: [[2026-09-23-fllwup-backlog-cleanup]],
[[2026-09-23-fllwup-epic-grouping]].

## EPIC-23 close-out and EPIC-24 grouping (2026-09-24)

A `Done` epic is not forced to have zero open children: EPIC-23 closed after its
two children merged while still carrying three `Backlog` `FLLWUP` children it had
filed; those were re-homed into a new EPIC-24 rather than left under a closed epic
([[follow-up-backlog-curation]]). The board stayed the single source of truth —
...
## EPIC-25 (2026-09-25) — the heading-uniqueness gap's third recurrence

The batch-override run ([[batch-runner-topology]]) closed five children in one
merge, then closed EPIC-25 `Done` and re-homed FLLWUP-122/123 to a new EPIC-26.
The board defect returned: its union-keep reconcile produced a **duplicate
`## In Progress` / `## In Review` header pair** with `validate.py` green; the
orchestrator repaired it by hand. This is the **third documented recurrence**
(EPIC-13 ×3, EPIC-15 residual run, now EPIC-25) of the gap this page already
calls "the obvious hardening… still no card". The status is therefore **open
and recurring**, not repaired. Witness: [[2026-09-25-epic25-run-ledger]].

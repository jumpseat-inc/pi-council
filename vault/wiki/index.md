...
- [[follow-up-backlog-curation]] — The periodic, human-directed maintenance of the accumulated `FLLWUP` backlog: content-preserving merges (keep the lower id, absorb verbatim), retirements that need a named reason (no `Retired` state exists), and the standing hazard that follow-up cards go stale silently.

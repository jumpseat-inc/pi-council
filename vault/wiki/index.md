...
- [[procedure-context-injection]] — EV-90 (EPIC-23): the `council-runner` dispatch input is composed from `renderProcedure`-substituted procedure bodies, removing the ~260 s per-card cold start; `$ARGUMENTS` is the card id in `council.md` but the epic key in `features-deliver.md`, so the composer binds two args.

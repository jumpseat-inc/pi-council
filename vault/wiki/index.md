...
- [[pi-council-overview]] — Pi-council as an installable package: the Council
  ...
  (through v0.37.0, released 2026-09-25; latest release v0.37.0. EPIC-6/7/8/9/13/14
  closed Done; EPIC-13 shipped metered deliberation routing at v0.21.0–v0.28.0,
  EPIC-14 re-homed gate enablement to `.council.json`, EPIC-10 shipped the typed
  follow-up review (File/Merge/Drop) at v0.33.0, and EPIC-23/24/25 closed Done
  through v0.37.0; EPIC-11/12 and EPIC-26 remain Backlog).
...
- [[council-runner]] — Autonomous per-card execution container for features-deliver; carries main-repo immutability + pinned verification subjects (v0.18.0). EPIC-15 found the follow-up review tool absent in-container (present since EPIC-23) and used an orchestrator-merges reading; since EPIC-23 the runner executes the merge and the orchestrator re-verifies it ([[deterministic-merge-check]] `Who executes the merge`); the EPIC-15 residual run found the recorded mode does not control execution ([[execution-mode-recording]]) and that the dispatch stall window must exceed the longest seat bound it waits on. EPIC-24's live smoke added the never-read rule and re-confirmed the stall-window invariant; EPIC-25 introduced the batch topology ([[batch-runner-topology]]).
...
- [[deterministic-merge-check]] — The features-deliver merge gate: five mechanical criteria observed directly, keyed since EPIC-13 by the card's recorded execution mode (Direct = 1/2/5; no recorded mode = HALT), merged --match-head-commit, Done only after gates green on the merged SHA. EPIC-15 used an orchestrator-merges reading; since EPIC-23 the runner executes the merge and the orchestrator re-verifies it. EPIC-24 added three Deliberate merges (PRs #114–#116).
- [[execution-mode-recording]] — The ROOT manifest `mode` is metadata, not a control: the runner re-derives its path and can execute a weaker mode than recorded, so a recorded mode stricter than execution HALTs the merge check; `readCardMode` upgrades to Deliberate on any generator seat, making a recorded `Direct` the robust default.
...
- [[metered-deliberation-routing]] — EPIC-13 (shipped v0.28.0): a typed System One gate routes each card to Deliberate/Verify/Direct; **`off` routes every card to the full Deliberate panel** (maximum scrutiny); enablement lives in `.council.json`'s top-level `gate` section — packaged default `mode: "off"`. EPIC-10 (2026-09-22) found the shipped gate inert in production (noul wire-shape drift, dead recorded-decision fast path; FLLWUP-99/100/104); EPIC-15 confirmed both gate domains inert and the run on the Deliberate fallback; EPIC-23 (2026-09-24) found the lanes still dark at read-back (`policyVersion` drift) — recorded modes across the corpus read Deliberate 131 / Direct 10 / Verify 1.
- [[followup-decision-gate]] — EPIC-10 (shipped v0.33.0): the follow-up review's typed File/Merge/Drop decision, a sibling of the card gate sharing one `gate.mode`; a failed decision falls back to the human and may never Drop or auto-Merge. EPIC-15 scoped: the review tool was absent in-container (candidates held and seat-ruled); the grant landed after and it is present/used since EPIC-23.
- [[follow-up-backlog-curation]] — The periodic, human-directed maintenance of the accumulated `FLLWUP` backlog: content-preserving merges (keep the lower id, absorb verbatim), retirements that need a named reason (no `Retired` state exists), then grouping the survivors into thematic close-out epics; plus the standing hazard that follow-up cards go stale silently. EPIC-24→EPIC-25 re-homed five residuals at closure in-run.
- [[inert-gate-fallback]] — An enabled decision gate is inert two ways: its live call fails, OR a recorded decision is dropped at read-back by schema/version drift — both fall back to the safe lane (Deliberate / human review) and the run's mode comes from the run substrate. EPIC-15 (live-call arm, noul, fixed 2026-09-22); EPIC-23 (read-back arm, `policyVersion` drift, FLLWUP-99). EPIC-24 a third read-back witness.
...
- [[phase1-rulings-record]] — EV-89 (EPIC-23): `/features-deliver` Phase 1 gains a durable, class-enumerated rulings record at `council/phase1-rulings.json` (`ruling` | `n/a: <reason>`), a `Phase 1 unresolved: <class>` refusal fence, and a FLLWUP-60-shaped authorization clause for the record push.
- [[procedure-context-injection]] — EV-90 (EPIC-23): the `council-runner` dispatch input is composed from `renderProcedure`-substituted procedure bodies, removing the ~260 s per-card cold start; `$ARGUMENTS` is the card id in `council.md` but the epic key in `features-deliver.md`, so the composer binds two args. EPIC-24's live smoke found the runner still read `features-deliver.md` at startup until a seat-level never-read rule.
- [[run-time-profile]] — The measured council/`features-deliver` time profile: ~52% model latency / ~44% tool, ~80% of tool time in serial single-child `council_wait`, ~1.4–1.7× session parallelism, median ~260 s runner cold start.
- [[halt-repair-gap]] — The command's under-specified HALT repair: no sanctioned path to clear a halt, so orchestrators improvise — the EPIC-15 mode mismatch and the EPIC-23 dispatch-window overrun.
- [[derived-key-refusal-posture]] — The D1 ruling: a derived key that is null/missing fails loud (throws); "byte-identical" in an acceptance criterion governs the enumerated fields, not the dispatch's occurrence.
...
- [[usages-report]] — `/usages <time_range>`: a packaged procedure + `/council-init`-copied tool reporting one repo's per-seat / main-agent token+dollar usage, cross-matched to OpenRouter as JSON + Markdown under `.pi/council/usages/`. EPIC-15 fixed the cache-before-mkdir ordering and added the stale-copy remediation route; the residual run added the foreign `--cache-file` parent creation (FLLWUP-109) and the `cache: hits=<N> misses=<M>` summary line (FLLWUP-110).
...
- [[2026-09-23-epic15-residual-run-ledger]] — The EPIC-15 residual run: five Backlog residuals (FLLWUP-111, 109, 110, 107, 108) delivered serially (PRs #107–#111) under run-scoped R-A/R-B authorizations; the recorded-mode-vs-executed-mode mismatch that HALTed FLLWUP-111; the held-follow-up routing to product-owner that filed FLLWUP-112/113.
- [[2026-09-23-fllwup-backlog-cleanup]] — The 2026-09-23 curation of the open follow-up backlog: 65 Backlog `FLLWUP` cards consolidated to 42 via 17 content-preserving merges and 2 retirements; the no-`Retired`-state constraint, the silent-staleness finding, and the cross-epic `epic: null` consequence.
- [[2026-09-23-fllwup-epic-grouping]] — The 2026-09-23 grouping of the 41 open follow-ups into 7 thematic close-out epics (EPIC-16…22): grouping repairs the `epic: null` orphans curation created, origin attribution becomes prose-only, and a close-out epic's goal is aggregate.

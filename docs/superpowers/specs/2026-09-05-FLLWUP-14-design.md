...
11. Host exit status is the frame verdict, never the prune's: with expired but unremovable run
    dirs in the artifacts tree (foreign-owned — root-owned from the Docker bind-mount path — or
    own-but-permission-blocked), a green run still exits 0 and prints "SMOKE PASS", having
    pruned everything it could; every unremovable entry is named in a visible `prune:` warning
    on stderr (foreign-owned vs own-permission branches), and a prune failure never inverts the
    verdict. Can-fail provable without root: an own `chmod 000` dir (with content) made the
    oldest prune candidate makes the old `xargs -r rm -rf` pipeline exit 123; the fixed
    harness exits 0 with the warning visible.
